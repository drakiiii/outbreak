...
def build_layout(layout: str, k: int):
    """Return (coordinates array (k,2), mobility matrix or None)."""
    if layout == "Line":
...
def build_metapop():
    ...
    layout = st.sidebar.selectbox("Layout", ["Line", "Ring", "Grid", "Random (all-to-all)"])
    ...
    coords, mobility = build_layout(layout, k)
    # Seed only the first region (the others start empty so we can watch arrival).
    regions = [
        Region(name=f"R{i+1}", population=int(pop),
    ...
    config = MetapopulationConfig(regions=regions, coupling=coupling,
                                  travel_rate=travel_rate, mobility=mobility)
    return base.validate(), config.validate(), int(seed)
...
def main():
    ...
    base, config, seed = build_metapop()
    ...
    day_idx = st.slider("Day", 0, len(days) - 1, min(len(days) - 1, len(days) // 3),
                        format="%d")
    st.plotly_chart(plot_map(coords, pops, names, mob, prevalence[:, day_idx], vmax),
